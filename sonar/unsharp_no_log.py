...
directory = '../datasets/gamma2rayleigh/trainA/'  # Directory containing noisy images
org_directory = '../datasets/gray_forest/trainA/'  # Directory containing corresponding original images
save_directory = '../datasets/unsharp_no_log/trainA/'  # Directory to save processed images
...
for (noisy_batch, noisy_filenames), (original_batch, original_filenames) in zip(
    ...
    for noisy, original, filename in zip(noisy_batch, original_batch, noisy_filenames):
        ...
        unsharp = unsharp_mask(noisy, radius=1.0, amount=1.0)
